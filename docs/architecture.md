...
- 上传 UTF-8 Markdown、文本型 PDF 和 DOCX 文档
- 手动触发分块与向量化
...
- ADMIN/USER 账号、管理员共同维护的公共知识库，以及按提问者隔离的会话和消息

当前不支持：

- 注册、账号找回、知识库共享范围配置和组织
- PDF、TXT、DOCX、OCR
- 异步文档处理任务
- 用户选择一个或多个知识库作为检索范围
- 消息队列
...
- 只有管理员可创建和管理知识库；所有管理员可共同维护。知识库 `owner_id` 保留创建者身份，异步文档处理继续使用该值。

只有管理员创建的知识库中，文档当前生效且状态为 `READY` 的版本可以参与检索。普通用户读取原文件时，`GET /api/conversations/{conversationId}/messages/{messageId}/sources/{citationId}/content` 校验本人会话、已完成回答和实际引用；管理用文档接口仅管理员可用。

### 会话

- `conversations`：标题、最多 400 字的纯文本话题摘要和摘要游标；升级时旧五字段 JSONB 摘要转为文本，旧列随后删除。
...
- 只检索 READY 的当前生效文档版本。
- 会话问答默认检索全部由管理员创建的知识库；local profile 的单轮兼容接口可显式限定知识库集合，但检索 SQL 仍按创建者的管理员角色校验。
- 规划模型最多生成 4 个子问题；简单问题保持一个子问题，非法规划稳定回退原问题。
- 会话规划后一次模型调用给全部启用的意图树叶子和所有子问题打分；叶子为 `KB`、`MCP`、`SYSTEM`，映射到现有 `KNOWLEDGE_RETRIEVAL`、`MCP_TOOL`、`SYSTEM_CHAT` 执行分支。树为空或分类失败时回退原有三类路由。MCP 默认关闭且允许列表为空，因此当前运行时不会执行外部工具。
- 知识库叶子用约 75% 的向量召回预算检索绑定库，其余约 25% 检索其他管理员创建的公共库；同一模型的查询向量仅生成一次。
...
- 提示词中的会话记忆、问题计划、工具观察和原始问题是互相分隔的不可信数据区；重排 Top K 后按文档版本聚合的知识证据使用纯文本 `<content ref="S*">` 边界，不发送内部来源元数据。
- `AnswerGenerator` 隔离模型客户端，`AnswerStage` 统一处理流式生成、候选回退、取消、引用校验和一次修复。
- sources 是实际进入最终提示词的文档级来源快照，包含拼接正文和所有入选分块的行位置；旧 JSON 快照惰性解码为单位置来源，原编号不变。
- citations 是回答中经过编号校验的文档来源子集，来源面板仅显示该子集；新回答引用在校验后归并到段落或列表项末尾，若流式正文发生变化则以 reset 和完整 delta 替换。
- Trace 的 `evidence_count` 仍统计入选证据分块数，而非聚合文档数。
...
摘要只记录具体话题的当时处理状态、用户仍有效的约束与偏好、未解决事项和关键指代，不复制历史知识库答案；“当时未找到资料”等状态不代表当前资料仍缺失。新摘要单行且最多 400 个 Unicode 字符；升级迁移保留旧摘要文本，后续增量更新会将其压缩为新格式。
...
- 端到端 TTFT 以首个成功发送的非空思考或正文增量为准，模型 TTFT 以最终有效回答模型尝试的首段思考或正文为准。
...
| 当前限制 | 后续方向 |
| --- | --- |
| 支持 Markdown、文本型 PDF 和 DOCX | 后续按需增加 OCR 与更多格式 |
