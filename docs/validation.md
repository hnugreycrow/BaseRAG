...
- 执行 `./mvnw.cmd clean test` 并在最终改动后复跑 `./mvnw.cmd test`，最终共 155 项：144 项通过，11 项基础设施测试因未设置 `RAG_INTEGRATION=true` 跳过，0 失败、0 错误。
- Trace 单元测试覆盖并发阶段顺序、span 单次终结、终态封存后的迟到回调、首 Delta 单次判定、最终回答模型 TTFT 和无内容空值。
- API/查询测试覆盖完整筛选参数绑定、`/summary` 静态路由、普通用户所有权限制与用户字段隐藏、管理员全局统计、跨用户详情 404、空样本及降级原因去重。
- 模型协议测试确认每个实际 HTTP 尝试均触发请求边界，首模型零输出失败后的候选正确标记为 `PROVIDER_FALLBACK`；子问题测试确认超时最终覆盖工作线程的成功回调。
- 新增的 PostgreSQL 条件测试覆盖会话删除后 Trace 外键置空保留，以及 30 天清理后阶段级联删除；本轮未启用基础设施环境，因此该项已编译但未实际执行。
- 本轮范围不含前端观测页面，也未对 `/api/questions` 增加 Trace；未运行前端构建。
