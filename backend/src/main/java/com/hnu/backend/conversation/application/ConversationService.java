...
import com.hnu.backend.conversation.api.ConversationResponses;
...
import com.hnu.backend.conversation.infrastructure.persistence.GenerationAttemptMapper;
...
import com.hnu.backend.question.application.RetrievalService;
...
public class ConversationService {
  ...
  public void cancel(UUID conversationId, UUID generationId) {
    ...
    if (active != null && active.conversation().getId().equals(conversationId)) active.cancel();
  }
  ...
  private void disconnect(ActiveGeneration active) {
    if (!active.terminal.get()) active.cancel();
  }
  ...
  private ChatClient.Generation streamAnswer(
    ...
    messages.markStreaming(active.assistant.getId());
    return chat.stream(
        system,
        prompt,
  ...
  private void complete(
    ...
    messages.complete(
        active.assistant.getId(), content, json.writeValueAsString(citations), modelInfo);
    conversations.touch(active.conversation.getId());
    active.assistant = messages.find(active.assistant.getId());
    finish(active, "complete", terminalPayload(active.assistant, active.requestId));
  }

  private void cancelTerminal(ActiveGeneration active) {
    if (active.terminal.get()) return;
    String content = active.buffer.toString();
    failCurrentAttempt(active, "CANCELLED", "GENERATION_CANCELLED", "生成已停止");
    messages.terminalFailure(
        active.assistant.getId(), "CANCELLED", content, "GENERATION_CANCELLED", "生成已停止");
    conversations.touch(active.conversation.getId());
    active.assistant = messages.find(active.assistant.getId());
    finish(active, "cancelled", terminalPayload(active.assistant, active.requestId));
  }

  private void errorTerminal(ActiveGeneration active, String code, String message) {
    if (active.terminal.get()) return;
    String content = active.buffer.toString();
    messages.terminalFailure(active.assistant.getId(), "FAILED", content, code, message);
    conversations.touch(active.conversation.getId());
    active.assistant = messages.find(active.assistant.getId());
    finish(active, "error", terminalPayload(active.assistant, active.requestId));
  }
  ...
  private void finish(ActiveGeneration active, String event, Object payload) {
    ...
    activeByGeneration.remove(active.assistant.getId(), active);
    try {
