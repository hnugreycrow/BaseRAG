...
import com.hnu.backend.conversation.service.ConversationGenerationService.ActiveGeneration;
...
public class ConversationGenerationRunner {
    ...
    try {
      ...
      active.channel.send(
          ...
          ConversationStreamEvents.started(
              ...
              active.assistant.getVariantIndex()));
      var prepared =
  ...
  private void ensureNotCancelled(ActiveGeneration active) {
    if (active.control.cancelled() || active.terminal.get()) {
      throw ApiException.cancelled();
    }
  }
  ...
  private final class ConversationAnswerObserver implements AnswerStage.Observer {
    ...
    public void normalizedAnswer(String content) {
      ...
      if (!active.reasoningBuffer.isEmpty()) {
        active.channel.send(
            Kind.REASONING_DELTA, ConversationStreamEvents.text(active.reasoningBuffer.toString()));
      }
