...
describe('upload document dialog', () => {
  it('accepts multiple Markdown extensions, rejects invalid files and limits the selection', async () => {
    ...
    try {
      ...
      await flushPromises()
      expect(wrapper.text()).toContain('请选择 .md、.markdown、.pdf 或 .docx')
      const first = uploadFile('one.md', 2)
      const second = uploadFile('two.markdown', 3)
      onChange(first, [first])
      onChange(second, [first, second])
      const pdf = uploadFile('three.pdf', 4)
      const docx = uploadFile('four.docx', 5)
      onChange(pdf, [first, second, pdf])
      onChange(docx, [first, second, pdf, docx])
      const oversized = uploadFile('huge.md', 6, 'x'.repeat(5 * 1024 * 1024 + 1))
      onChange(oversized, [first, second, pdf, docx, oversized])
      await flushPromises()
      ...
      expect(wrapper.emitted('submit')?.[0]).toEqual([[first.raw, second.raw, pdf.raw, docx.raw]])
