...
- 分块先按 Markdown 结构和自然句界识别语义块，再合并短章节；默认目标 1400、最小 500、最大 2000、超长段落重叠 180 字符。Top K=5，问题最多 2000 字符。应用不裁剪模型上下文，聊天请求也不发送 max_tokens；实际模型上下文上限仍由服务商决定。
...
- 同一文件重复上传会产生新文档；尚无 PDF、去重、版本替换或权限。当前会话仍是 local 单用户数据空间。
