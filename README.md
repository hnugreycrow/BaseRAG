...
- 知识库与文档版本固定保存向量模型配置 ID、供应商、模型名及维度；同 ID 的供应商、模型名或维度被改写，或模型从配置中移除时，会拒绝分块和问答，不会跨供应商混用旧向量或自动故障转移。恢复原配置可继续使用；需要换模型时保留旧数据并新建知识库后重新分块。
- 新建知识库默认使用百炼 `bailian-qwen3.7-embedding`（1536 维）；原有知识库仍固定使用原硅基流动 `qwen-emb-8b` 绑定，不会因默认模型变化而重建或混用向量。百炼 Chat/Rerank 使用 `bailian.url`，可选 `BAILIAN_EMBEDDING_BASE_URL` 仅覆盖该 Embedding 候选地址。两家 Embedding 均走 OpenAI 兼容接口，无需配置协议字段。
