...
- 默认仍使用硅基流动 `qwen-emb-8b`。配置 `BAILIAN_API_KEY` 后，新建知识库可选百炼 `bailian-qwen3.7-embedding`（1536 维）；可选 `BAILIAN_EMBEDDING_BASE_URL` 仅覆盖其 Embedding 地址，百炼 Chat/Rerank 继续使用共享地址。两家 Embedding 均走 OpenAI 兼容接口，无需配置协议字段。
